...
{
    /// <summary>Implementation of a ziggurat sampler for producing gaussian distributions from uniform ones.</summary>
    public sealed class Ziggurat : IGaussianRandom
    {
        ...
        /// <summary>
        ///     Right hand x coord of the base rectangle, thus also the left hand x coordinate of the tail
        ///     (pre-determined/computed for 128 blocks).
        /// </summary>
        private const double R = 3.442619855899;

        /// <summary>Area of each rectangle (pre-determined/computed for 128 blocks).</summary>
        private const double A = 9.91256303526217e-3;

        /// <summary>Scale factor for converting a UInt with range [0,0xFFFFFFFF] to a double with range [0,1].</summary>
        ...
        private readonly IUniformRandom _rng;
        ...
        private readonly double _aDivY0;

        #endregion

        #region Constructors

        /// <summary>Construct with a default RNG source.</summary>
        public Ziggurat()
            : this(new MersenneTwister()) {}

        /// <summary>Construct with the specified RNG seed.</summary>
        ...
        /// <summary>Get the next sample value from the gaussian distribution, but clamp it to the 2*sigma interval around mu.</summary>
        /// <param name="mu">The distribution's mean.</param>
        /// <param name="sigma">The distribution's standard deviation.</param>
        ...
        /// <summary>Sample from the distribution tail (defined as having x >= __R).</summary>
        /// <returns></returns>
        ...
        /// <summary>Gaussian probability density function, denormalised, that is, y = e^-(x^2/2).</summary>
        private static double GaussianPdfDenorm(double x)
        {
            return System.Math.Exp(-(x * x / 2.0));
        }

        /// <summary>Inverse function of GaussianPdfDenorm(x)</summary>
