...
namespace Space.ComponentSystem.Components
{
    class Radar:AbstractComponent
    {
        ...
        public override void Update(object parameterization)
        {
            ...
            //get radar modules
            var radar = modules.GetModules<RadarModule>();
            float range = 0;
            if (radar != null)
            {   //get sum of radars todo check if we need multiple
                range += radar.Sum(radarModule => radarModule.RadarDistance);
            }
            //calculate distance
            range = modules.GetValue(EntityAttributeType.RadarDistance, range);

            foreach (var neigbour in index.GetNeighbors(Entity, range))
            {
                if (neigbour == null || neigbour.GetComponent<Transform>() == null) continue;
