...
namespace ProjectMercury.Emitters
{
    ...
    public class Emitter
    {
        ...
        /// </summary>
        private FarValue MostRecentTrigger;
        ...
        public void Update(float deltaSeconds)
        {
            ...
            unsafe
            {
                ...
                fixed (Particle* particleArray = this.Particles)
                {
                    ...
                    while (--i >= 0)
                    {
                        float actualAge = (float)(this.TotalSeconds - particle->Inception);
        ...
        public void Trigger(ref FarPosition triggerPosition, float rotation = 0.0f, float scale = 1.0f)
        {
            var zero = Vector2.Zero;
            Trigger(ref triggerPosition, ref zero, rotation, scale);
        }
        ...
        /// </summary>
        /// <exception cref="System.InvalidOperationException">Thrown if the Emitter has not been initialised.</exception>
        public void Trigger(ref FarPosition triggerPosition, ref Vector2 impulse, float rotation = 0.0f, float scale = 1.0f)
        {
            Guard.IsFalse(this.Initialised, "Emitter has not been initialised.");
                ...
                if (i < this.Budget)
                {
                    unsafe
                    {
                        fixed (Particle* particle = &this.Particles[i])
                        {
                            ...
                            particle->Momentum = this.ReleaseImpulse * scale + impulse;
                            particle->Age = 0f;
                            particle->Colour = new Vector4(this.ReleaseColour.Sample(), this.ReleaseOpacity.Sample());
                            particle->Scale = this.ReleaseScale.Sample() * scale;
