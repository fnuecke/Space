...
namespace Engine.Network
{
    ...
    /// </remarks>
    public sealed class DeprecatedUdpProtocol : IProtocol
    {
        #region Constants
        ...
        /// </summary>
        private static Dictionary<ushort, DeprecatedUdpProtocol> loopbacksByPort = new Dictionary<ushort, DeprecatedUdpProtocol>();
        ...
        public DeprecatedUdpProtocol(ushort port, byte[] protocolHeader)
        {
            ...
            switch (type)
            {
                ...
                case SocketMessage.Acked:
                    ...
                    if (connection != null)
                    {
                        ...
                        List<Packet> readyMessages;
                        if (!connection.PushMessage(data, messageNumber, out readyMessages))
                        {
                            if (readyMessages == null)
                            {
                                // Nothing to do, no new messages.
                                return;
                            }
                            foreach (var readyMessage in readyMessages)
                            {
                                ...
                                readyMessage.Rewind();
                                var dataArgs = new ProtocolDataEventArgs(remote, readyMessage);
                                OnData(dataArgs);
        ...
        private void Send(byte[] dgram, int bytes, IPEndPoint endPoint)
        {
            ...
            {
                DeprecatedUdpProtocol remote = loopbacksByPort[(ushort)endPoint.Port];
    ...
    sealed class UdpRemoteInfo
    {
        ...
        /// <summary>
        /// The last sequenced message we received.
        /// </summary>
        private int lastMessageNumber = -1;

        /// <summary>
        /// List of sequenced messages we're holding back until the message
        /// before them arrives.
        /// </summary>
        private Dictionary<int, Packet> queuedMessages = new Dictionary<int, Packet>();
        ...
        /// <summary>
        /// Push a received message to the list of received messages, and get
        /// a list of any messages that might be ready for handling now.
        /// </summary>
        /// <param name="message">the message that was received.</param>
        /// <param name="messageNumber">the number of the received message.</param>
        /// <param name="readyMessages">a list of messages that are ready for handling now.</param>
        /// <returns></returns>
        public bool PushMessage(Packet message, int messageNumber, out List<Packet> readyMessages)
        {
            readyMessages = null;
            ...
            // Not yet handled. If this is the next one, mark it as ready and
            // also look for any other messages that might follow it.
            // Then try to fetch the next sequenced message from the queue.
            if (messageNumber == lastMessageNumber + 1)
            {
                int readyMessageNumber = lastMessageNumber + 2;
                readyMessages = new List<Packet>();
                readyMessages.Add(message);
                // Now find following messages.
                while (queuedMessages.ContainsKey(readyMessageNumber))
                {
                    readyMessages.Add(queuedMessages[readyMessageNumber++]);
                }
            }
            else if (!queuedMessages.ContainsKey(messageNumber))
            {
                queuedMessages.Add(messageNumber, message);
            }
            return false;
        }

        /// <summary>
        /// Mark a message with the given number as handled.
        /// </summary>
        /// <param name="messageNumber">the number of the message to mark as handled.</param>
        public void MarkHandled(int messageNumber)
        {
            queuedMessages.Remove(messageNumber);
