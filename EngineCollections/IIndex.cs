...
{
    /// <summary>
    /// A callback method for circle and area index queries. Callbacks will be executed
    /// for each found hit, and search will continue based on the returned value.
    /// </summary>
    ...
    /// <returns>Whether the search should continue (<c>true</c>) or not (<c>false</c>).</returns>
    public delegate bool SimpleQueryCallback<in T>(T value);

    /// <summary>
    /// A callback method for line index queries. Callbacks will be executed  for each
    /// found hit, and search will continue based on the returned value. The line
    /// callback method may change the fraction value, to adjust the search space.
    /// - If a negative value is returned, the result is ignored and the maximum
    ///   fraction will not change.
    /// - If zero is returned the search is stopped.
    /// - Otherwise the returned value is taken as the new maximum fraction, i.e.
    ///   hits further away will be ignored.
    /// </summary>
    /// <typeparam name="T">The type of value stored in the index.</typeparam>
    /// <param name="value">The value that the query found a hit for.</param>
    /// <param name="fraction">The fraction to allow future hits in.</param>
    /// <returns>
    /// The new maximum fraction up to which to search. If set to less or equal to
    /// zero the search will be stopped.
    /// </returns>
    ...
    public interface IIndex<T, TRectangle, in TPoint> : IEnumerable<Tuple<TRectangle, T>>
    {
        ...
        /// in the index that are contained in or intersecting with the specified
        /// query rectangle.
        /// </summary>
