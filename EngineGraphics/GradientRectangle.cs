...
{
    /// <summary>Utility class for rendering filled gradient rectangles, interpolating between multiple colors.</summary>
    public sealed class GradientRectangle : AbstractShape
    {
        #region Fields

        /// <summary>The colors to use when blending.</summary>
        private readonly Vector4[] _colors = new Vector4[3];

        /// <summary>The points on the interval from and to which to interpolate.</summary>
        private readonly float[] _points = new float[3];

        /// <summary>The actual number of points currently in use.</summary>
        ...
        /// <summary>Creates a new gradient rectangle renderer for the given game.</summary>
        ...
        /// <summary>
        ///     Sets the gradient points with their colors to use for interpolating the color over the size of this rectangle.
        ///     <para>
        ///         A minimum of two points and two colors is required, and the number of colors has to be same as the number of
        ///         points. The colors will then be interpolated between the relative positions of the points.
        ///     </para>
        /// </summary>
        ...
        /// <summary>
        ///     Sets the gradient points with the colors to use for interpolating the color over the size of the rectangle.
        ///     <para>A minimum of two colors is required. The points will be automatically generated by linear interpolation.</para>
        /// </summary>
        /// <param name="colors">The colors between which to interpolate.</param>
        public void SetGradients(Color[] colors)
        {
            ...
            {
                _colors[i] = colors[i].ToVector4();
                _points[i] = MathHelper.Lerp(0f, 1f, i / (float) (_pointCount - 1));
            }
        }
        ...
        protected override void AdjustParameters()
        {
            ...
            var valueCount = Effect.Parameters["NumValues"];
            if (valueCount != null)
            {
                valueCount.SetValue(_pointCount);
            }
